...
impl Player {
    ...
    pub async fn play(&self) -> Result<()> {
        ...
        if let Some(current_track_id) = tracklist.currently_playing() {
        ...
        if chrono::Utc::now() - *CURRENT_TRACK_URL_TIME.read().await > TRACK_URL_LIFE_SPAN {
            ...
            let client = get_client().await;
            if let Some(track_id) = tracklist.currently_playing() {
                ready().await?;
        ...
        match msg.view() {
            ...
            MessageView::StreamStart(_) => {
                ...
                if is_player_playing().await {
                    ...
                    let tracklist = self.tracklist.read().await.clone();
                    self.broadcast_track_list(tracklist.clone()).await?;
                }
            }
    ...
    pub async fn play_album(&mut self, album_id: &str, index: u32) -> Result<()> {
        ...
        tracing::info!("Fechting album: {}", album_id);
        let album: Album = client.album(album_id).await?.into();
        tracing::info!("Got album: {}", album.title);
        ...
        if let Some(track) = tracklist.skip_to_track(index - unstreambale_tracks_to_index) {
            ...
            tracing::info!("Got first track url: {}", track.title);
            query_track_url(&track_url).await?;
            tracing::info!("track got query: {}", track.title);
